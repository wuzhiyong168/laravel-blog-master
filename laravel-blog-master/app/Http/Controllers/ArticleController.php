...
class ArticleController extends Controller
{
  ...
  {
    $articles = Article::where('is_hidden', 0)->orderBy('created_at', 'desc')->paginate(5);
    for ($i=0; $i < sizeof($articles); $i++) {
      $articles[$i]->content = str_limit(strip_tags($articles[$i]->content), 50);
  ...
  public function search(Request $request)
  {
    ...
    })->where('is_hidden', 0)->orderBy('created_at', 'desc')->paginate(5);
    for ($i=0; $i < sizeof($articles); $i++) {
      $articles[$i]->content = str_limit(strip_tags($articles[$i]->content), 50);
      $articles[$i]->created_at_date = $articles[$i]->created_at->toDateString();
      $articles[$i]->updated_at_diff = $articles[$i]->updated_at->diffForHumans();
